...
def prometheus_log(results):
    ...
    for i in range(0, len(results)):
        ...
        print(f"hall_temperature{i} {results[i]['temp_c']}", flush=True, file=metrics_out)
        print(f'# HELP pressure{i} something', flush=True, file=metrics_out)
        print(f'# TYPE pressure{i} gauge', flush=True, file=metrics_out)
        print(f'hall_pressure{i} 0', flush=True, file=metrics_out)
        ...
        print(f"hall_humidity{i} {results[i]['humidity']}", flush=True, file=metrics_out)
...
def database_setup():
    ...
    if connection_id is not None:
        ...
        db_add_sensor(connection_id, ('hallway', "DHT22", DHT_PIN_1))
    else:
