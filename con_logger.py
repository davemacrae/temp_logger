...
DHT_PIN_2 = 24                      # GPIO PIN for Sensor 2
...
def prometheus_log(results):
    ...
    for i in range(0, len(results)):
        ...
        print(f"con_temperature{i} {results[i]['temp_c']}", flush=True, file=metrics_out)
        print(f'# HELP pressure{i} something', flush=True, file=metrics_out)
        print(f'# TYPE pressure{i} gauge', flush=True, file=metrics_out)
        print(f'con_pressure{i} 0', flush=True, file=metrics_out)
        ...
        print(f"con_humidity{i} {results[i]['humidity']}", flush=True, file=metrics_out)
